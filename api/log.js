// api/log.js — Proxy to Google Apps Script (ESM)
...
export default async function handler(req, res) {
  ...
  if (!scriptUrl) return res.status(500).json({ ok: false, error: 'Missing APPS_SCRIPT_URL env var' });
  ...
  if (!consent) return res.status(200).json({ ok: true, skipped: 'no-consent', ts: new Date().toISOString() });
